	        ...
	        <div class="col-lg-12">
	          <div class="card">
	            ...
	            <div style="width: 100%;text-align: right;padding-right: 20px;padding-top: 10px;">
                        <a href="{{ url('admin/addnewabout')}}"><button class="btn btn-info btn-round">Add New</button></a>
                      </div>
	            <div class="card-body">
	              ...
	              <table id="example" class="table table-bordered" style="text-align: center;">
	                ...
	                <tbody>
	                    <?php $count = 1; if(isset($aboutus_list) && !empty($aboutus_list)){ ?>
                        @foreach ($aboutus_list as $banner)
                         <tr>
                           <td>{{ $count++ }}</td>
                           <td>{{ $result = mb_substr( $banner->Title, 0,10) }}..  </td>
                      	   <td><p>{{ $result = mb_substr( $banner->Description, 0,10) }}.. </p></td>
                           <td><img src="{{ url('')}}/{{ $banner->Photo }}" width="200px" height="70px"></td>
                           <td>
                              <a href="{{ url('admin/edit_about/'.$banner->UniqueId) }}"><button class="btn btn-info btn-round">Edit</button></a>
                               <a href="{{ url('admin/delete_about/'.$banner->UniqueId) }}"><button OnClick="return confirm('Are You Sure Want to Delete?')" class="btn btn-danger btn-round">Delete</button></a>
                           </td>
                        </tr>
                         @endforeach
                         <?php } ?>
	 ...
	 <?php } ?>
	      ...
	      <div class="row">
				<div class="col-lg-12">
				   <div class="card">
				     <div class="card-body">
					   <div class="card-title text-dark">New About US </div>
					   <hr>
					    <form enctype="multipart/form-data" id="" method="post" action="{{ url('/admin/add_about') }}">
	                     <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
						
						 <div class="form-group row">
						  <label for="input-21" class="col-sm-2 col-form-label">About Us Title </label>
						  <div class="col-sm-10">
						 ...
						 <div class="form-group row" style="text-align: center;">
						  ...
						  <div class="col-sm-10">
							<button type="submit" value="submit" class="btn btn-info shadow-dark px-3 submit"> Save</button>
								<a href="{{ url('/admin/about') }}" type="reset" name="reset" class="btn btn-danger shadow-dark px-3">Cancel</a>
						  </div>
						</div>
						</form>
					 </div>
     ...
     <?php if(isset($edit)){ ?>
	      <div class="row">
				<div class="col-lg-12">
				   <div class="card">
				     <div class="card-body">
					   <div class="card-title text-dark">Edit About US </div>
					   <hr>
					    <form enctype="multipart/form-data" id="" method="post" action="{{ url('/admin/edit_about/'.$c[0]->UniqueId) }}">
	                     <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
